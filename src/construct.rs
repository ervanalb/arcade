...
pub fn segment_from_two_points(p0: Trivector, p1: Trivector) -> Curve {
    let line = p0 & p1;
    ...
    Curve::TrimmedCurve(TrimmedCurve {
        curve: Box::from(Curve::Line(Line {p0: p0, d: direction})),
        t_start: 0.,
        t_end: length,
    })
}

pub fn arc_from_three_points(p0: Trivector, p1: Trivector, p2: Trivector) -> Curve {
    ...
    Curve::TrimmedCurve(TrimmedCurve {
        curve: Box::from(Curve::Circle(Circle {p0: p0, a: axis})),
        t_start: 0.,
        t_end: angle,
    })
}
